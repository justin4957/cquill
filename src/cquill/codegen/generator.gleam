...
pub fn with_typed_columns(
  ...
  GeneratorConfig(..config, generate_typed: enabled)
}
...
pub fn generate_all(
  ...
  // Generate typed column modules if enabled
  let typed_modules = case config.generate_typed {
    True -> {
      let typed_table_modules =
        tables
        |> list.map(fn(table) { generate_typed_module(table, enums, config) })
      let typed_index = [generate_typed_index_module(tables, config)]
      list.flatten([typed_table_modules, typed_index])
    }
    False -> []
  }

  list.flatten([table_modules, enum_module, index_module, typed_modules])
}
...
// ============================================================================
// TYPED COLUMN GENERATION
// ============================================================================

/// Generate a typed column module for a table
...
pub fn generate_typed_module(
  ...
  let enum_names = list.map(enums, fn(e) { e.name })
  let columns = map_columns(table, enum_names, config)
  let type_name = pascal_case(table.name) <> "Table"
  let module_name = snake_case(table.name)
  let table_fn_name = snake_case(table.name)

  let content =
    generate_typed_module_header(table.name, config)
    <> "\n"
    <> generate_typed_imports(columns)
    <> "\n"
    <> generate_phantom_type(type_name)
    <> "\n"
    <> generate_table_constant(table, table_fn_name, type_name)
    <> "\n"
    <> generate_typed_columns(columns, type_name)

  GeneratedModule(
    path: config.module_prefix <> "/typed/" <> module_name,
    filename: module_name <> ".gleam",
    content:,
  )
}

fn generate_typed_module_header(
  table_name: String,
  _config: GeneratorConfig,
) -> String {
  "// "
...
fn generate_typed_imports(columns: List(ColumnMeta)) -> String {
  let imports = type_mapping.collect_imports(columns)

  // Base import for table and column types
  let base_import =
    "import cquill/typed/table.{type Table, type Column, table, column}"

  // Collect additional type imports needed for column value types
  let type_imports =
    imports
    |> list.filter_map(fn(imp) {
      case imp {
  ...
  let all_imports = [base_import, ..type_imports]
  string.join(all_imports, "\n")
}

fn generate_phantom_type(type_name: String) -> String {
  "/// Phantom type for " <> type_name <> " table\n" <> "pub type " <> type_name
}

fn generate_table_constant(
  table: IntrospectedTable,
  fn_name: String,
  type_name: String,
) -> String {
  "/// Get a typed reference to the "
  <> table.name
  <> " table\n"
  <> "pub fn "
  <> fn_name
...
fn generate_typed_columns(
  columns: List(ColumnMeta),
  type_name: String,
) -> String {
  columns
  |> list.map(fn(col) { generate_typed_column(col, type_name) })
  |> string.join("\n\n")
}

fn generate_typed_column(col: ColumnMeta, type_name: String) -> String {
  let col_name = col.column_name
  let value_type = render_type(col.gleam_type)
...
pub fn generate_typed_index_module(
  ...
  let content =
    generate_typed_index_module_header(config)
    <> "\n"
    <> generate_typed_index_imports(tables, config)

  GeneratedModule(
    path: config.module_prefix <> "/typed",
    filename: "typed.gleam",
    content:,
  )
}

fn generate_typed_index_module_header(_config: GeneratorConfig) -> String {
  "// typed.gleam (GENERATED)\n"
  <> "//// Typed table and column definitions for type-safe queries\n"
  <> "//// Generated by cquill - DO NOT EDIT\n"
}

fn generate_typed_index_imports(
  tables: List(IntrospectedTable),
  config: GeneratorConfig,
) -> String {
  tables
  |> list.map(fn(table) {
    let module_name = snake_case(table.name)
    "import " <> config.module_prefix <> "/typed/" <> module_name
  })
  |> string.join("\n")
}
